...
private static Logger logger = Logger.getLogger(FormVanzator.class.getPackage().getName());
	...
	public void setVanzator(Vanzator v) {
		logger.debug("Changed proiect : " + v.getCodAcces() + " :: " + p.getNume());
		this.Vanzator = v;
	...
	public Object getAsObject(FacesContext ctx, UIComponent uicomp, String uival) {
		...
		if (uicomp.getCodAcces().equals("cboVanzatori")){
		
			Integer codAcces = new Integer(uival);
			Vanzator v = new Vanzator();
			c.setCodAcces(codAcces);
	...
	public void validate(FacesContext arg0, UIComponent uiComponent, Object uiValue)
		...
		if ("id".equals(uiComponent.getId())){
			System.out.println("Validare id Vanzator");
			String idVanzator = uiValue.toString();
			FacesMessage mesaj = null;
			if (idVanzator == null || id.isEmpty()){
				mesaj = new FacesMessage("Idul Vanzatorii trebuie completat!");
			}
			System.out.println("Id: " + idVanzator + ": " + idVanzator.substring(0, 1));
