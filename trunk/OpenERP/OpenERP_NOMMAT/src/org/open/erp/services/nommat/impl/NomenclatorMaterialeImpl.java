	...
	public Material cautareMaterialDupaDenumire(String denumire) {
		...
		while (cmat.hasNext()){ 
			if(cmat.next().getDenumireMaterial()==denumire){
				...
				*/
			Material returnMaterial= new Material(cmat.next().getCodMaterial(),cmat.next().getDenumireMaterial(),cmat.next().getCantitateStandard(),cmat.next().getCategorieMaterial());
