...
public class Test {
	...
	public void setUp() {
		try {
			...
			m1 = new Material(1, "den mat 1", "uM");
			m2 = new Material(2, "den mat 2", "uM");
			m3 = new Material(3, "den mat 3", "uM");
			m4 = new Material(4, "den mat 3", "uM");
			m5 = new Material(5, "den mat 3", "uM");
			...
			// BonConsum bon =
			// (BonConsum)proc.preoceseazaComandaMateriale(comMat);
	...
	@org.junit.Test
	public void metodaTestIntrareStocProdusNou() {
		logger.loggeazaINFO("START test IntrareStocProdusNou------ ");
		// 4)---intrare cantitate fara a fi un articol in
		// stoc( un nou articol se creaza pt mterialul corespunzator)
		stocuriSRV.intrareInStoc(m4, new LoturiIntrari(55, 33, 5.5, new Date(),
				null), gst1);

		assertEquals("nu s-a creat nici un articol nou in gestiunea gst1", 4,
				gst1.getArticole().size());
		logger.loggeazaINFO("END test IntrareStocProdusNou------ ");
	}
