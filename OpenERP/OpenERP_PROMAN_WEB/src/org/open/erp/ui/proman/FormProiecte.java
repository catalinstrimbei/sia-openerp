...
@ManagedBean(name="formProiecte")
@SessionScoped
public class FormProiecte implements Converter{
	/* Resurse test*/
	private static Logger logger = Logger.getLogger(FormProiecte.class.getPackage().getName());
	...
	private ProjectManagementSrv promanInstance;
	
	/* Data Model */
	private List<Proiect> proiecte = new ArrayList<Proiect>();
	...
	private void initForm(){
		logger.debug("PostConstruct FORM PROIECTE local-proman: ..." + this.promanInstance);

		this.proiecte = promanInstance.getProiecte();
		if (!proiecte.isEmpty())
